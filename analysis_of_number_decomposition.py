...
# Тестові числа різної довжини
test_numbers = [
    10**5 + 3,  # 6-значне число
    10**10 + 19,  # 11-значне число
    10**15 + 37,  # 16-значне число
    10**20 + 51,  # 21-значне число
]
...
for n in test_numbers:
    duration, is_prime = time_function(solovay_strassen, n)
    results[f"Solovay-Strassen({n})"] = duration

# Факторизація
for n in test_numbers:
    duration, factors = time_function(full_pollard_factorization, n)
    results[f"Pollard Factorization({n})"] = duration

    duration, factors = time_function(trial_division, n)
    results[f"Trial Division({n})"] = duration

    duration, factors = time_function(drillhart_morrison, n)
    results[f"Brillhart-Morrison({n})"] = duration

results

# Вивід результатів
for key, value in results.items():
    print(f"{key}: {value:.6f} сек")
